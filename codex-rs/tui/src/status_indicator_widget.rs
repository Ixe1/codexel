...
impl Renderable for StatusIndicatorWidget {
    fn desired_height(&self, width: u16) -> u16 {
        1u16.saturating_add(u16::try_from(self.wrapped_details_lines(width).len()).unwrap_or(0))
            .saturating_add(u16::try_from(self.detail_lines.len()).unwrap_or(u16::MAX))
    }
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        let mut remaining = usize::from(area.height.saturating_sub(1));
        if remaining > 0 {
            // If there is enough space, add the details lines below the header.
            let details = self.wrapped_details_lines(area.width);
            let take = details.len().min(remaining);
            lines.extend(details.into_iter().take(take));
            remaining = remaining.saturating_sub(take);
        }
        if remaining > 0 {
            lines.extend(self.detail_lines.iter().take(remaining).cloned());
        }
